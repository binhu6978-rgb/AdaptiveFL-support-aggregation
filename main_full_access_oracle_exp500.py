"""Fixed-configuration 500-round Full-access Oracle experiment entry point."""
...
def main():
    ...
    args.full_access_oracle = True
    args.experiment_result_dir = os.path.join('results', 'full_access_oracle_full500')
    args.device = torch.device(
        'cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')

    os.makedirs(args.experiment_result_dir, exist_ok=True)
    original_stdout = sys.stdout
    tee = Tee(os.path.join(args.experiment_result_dir, 'full_access_oracle_500.log'))
    sys.stdout = tee
    started = time.time()
    try:
        print('Full-Access Oracle diagnostic: 500 rounds, all-profile evaluation')
        print('aggregation=Aggregation_AdaptiveFL; full_access_oracle=True; device={}'.format(args.device))
